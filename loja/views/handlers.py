...
def produto_json(request, pk):
    ...
    if hasattr(Produto, "empresa_id") and getattr(request, "empresa", None):
        if p.empresa_id and p.empresa_id != request.empresa.id:
            return JsonResponse({"detail": "Produto não pertence à empresa atual."}, status=403)
...
def _recalcular_totais(venda: Venda):
    tot = venda.itens.all().aggregate(total_sub=models.Sum("subtotal"), total_custo=models.Sum("custo_total"))
    ...
    if request.method == "POST":
        ...
        if f.is_valid():
            ...
            if not cliente:
                ...
                if nome:
                    cliente = Cliente.objects.create(
                        ...
                        empresa=getattr(request, "empresa", None) if hasattr(Cliente, "empresa_id") else None,
                    )
            ...
            venda = Venda.objects.create(
                ...
                total=0,
                custo_total=0,
                empresa=getattr(request, "empresa", None) if hasattr(Venda, "empresa_id") else None,
            )
...
def venda_detalhe(request, pk):
    ...
    return render(request, "loja/venda_detalhe.html", {"venda": venda, "add_form": add_form})
...
def loja_home(request):
    return redirect("loja:venda_nova")
